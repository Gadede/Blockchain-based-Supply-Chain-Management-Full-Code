...
contract supplychain is usingProvable{
    ...
    function getCertificateFromIPFSviaProvable(string memory cert_IPFS_Hash) public payable {
        message("Provable query to get Cert on IPFS sent, waiting for the answer...");
    ...
    function __callback(bytes32 myid, string memory result, bytes memory proof) public override {
       ...
       if(provable_randomDS_proofVerify__returnCode(myid, result, proof) == 0){
            ...
            certReceived("Cert received from IPFS", result); // Emit event.
        }
        else{
            // Handle failed proof verification.
            message("Failed Provable verification.");
        }
   }
}