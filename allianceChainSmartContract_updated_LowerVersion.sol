// Alliance chain Contract using lower compiler version.
...
contract certificate is usingOraclize {
    ...
    function applyForCert (string memory productID, string memory productDescription, string memory IPFS_Hash, string memory Origin, string memory farmerSig, address public_blockchainAddress) public returns(bool){
      require (bytes(checkValidator[productID].validator).length == 0); //check if productid exist by checking if it has an address
       ...
       Validator_Alert(productID, productDescription, IPFS_Hash, Origin, selectedValidator, public_blockchainAddress, farmerSig); //aert validator
       return true;
    }
   function validate(string memory productID, string memory cert_IPFS_Hash, uint8 cert_Status) public returns(bool){
       require (parseAddr(checkValidator[productID].validator) == msg.sender);
       ...
       if (cert_Status == 1) {
            farmerAlert (productID, "Approved Product");
       }else{
           farmerAlert ( productID,"Prodcut Denied");
       }
       return true;
   }
    ...
    function delValidator(string memory validator) public  returns(uint8){
     ...
     function find(string memory addr) internal view returns(uint) {
        uint i = 0;
        while (keccak256(validators[i]) != keccak256(addr)) {
            i++;
        }
        return i;
    }
